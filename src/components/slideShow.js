...
import { interiorImages } from './importImages';
...
function SlideShow({ images }) { 
  ...
  return (
    <div className='flex justify-between flex-col lg:px-32 lg:flex-row w-full lg:w-4/5'>
        <div ref={slideShowRef} className="slideShow show flex-col lg:flex-row w-full justify-end flex lg:mt-10 py-0 lg:py-5 border-primary rounded-lg bg-darker fade-in">
        <div className="sliders w-full h-500px object-contain">
          <SmartSlider slides={slidesArray} buttonShape="square" autoSlide={true} showIndicators={false} autoSlideInterval={10000}/>
        </div>
      </div>
    </div>
  );
